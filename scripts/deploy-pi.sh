...
sudo -u mcp-gateway mkdir -p "$candidate"
sudo -u mcp-gateway tar -xzf "$archive" -C "$candidate"
if [ -f "$current/config/targets.local.json" ]; then
    sudo -u mcp-gateway mkdir -p "$candidate/config"
    sudo -u mcp-gateway cp -p "$current/config/targets.local.json" "$candidate/config/targets.local.json"
fi
